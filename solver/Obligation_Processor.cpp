...
int Obligation_Processor::get_lowest_layer_for_initial_state() {
  return get_lowest_satisfying_layer(Compressed_State(Global::problem.initial_state, 0, true), INT_MAX);
}
...
int Obligation_Processor::get_lowest_satisfying_layer(const Compressed_State& state, int upper_known_satisfying_layer) {
  ...
  //if (upper_known_satisfying_layer == INT_MAX) ensure_solver_exists_for_end_reason_layer(_k);
  //else                                         ensure_solver_exists_for_end_reason_layer(upper_known_satisfying_layer);

  ensure_solver_exists_for_end_reason_layer(_k);
  ...
  int lowest_satisfying_layer = _k+1;
  for (int i=1;i<=_k;i++) {
