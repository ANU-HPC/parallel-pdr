...
Obligation::Obligation(
    const Compressed_State& compressed_state, 
    int layer, 
    int subproblem, 
    int or_originating_layer, 
    int or_count, 
    bool reduce_reason_add_successor_to_queue, 
    const vector<int>& banned_actions) {
...
Obligation Obligation::get_with_incremented_layer_and_or_count(int layer_amount, int or_count_amount) const {
  return Obligation(_compressed_state, _layer+layer_amount, _subproblem, _or_originating_layer, _or_count+or_count_amount, _reduce_reason_add_successor_to_queue, _banned_actions);
}
...
Obligation get_with_additional_banned_action(int extra_banned_action) const {
  ...
  return Obligation(_compressed_state, _layer, _subproblem, _or_originating_layer, _or_count+or_count_amount, _reduce_reason_add_successor_to_queue, new_banned_actions);
}
...
void Obligation::get_as_MPI_message(int* data, int start) const {
  ...
  data[start+5] = state_size;
  _compressed_state.get_as_MPI_message(data, start+6);

  Utils::get_as_MPI_message(data, start+7+state_size, _banned_actions) {
}
...
Obligation::Obligation(int* data, int start, int stop) {
  ...
  const int state_size = data[start+5];
  _compressed_state = Compressed_State(data, start+6, start+6+state_size);

  _banned_acttions = Utils::get_from_MPI_message(data, start+8+state_size, stop);
}
...
int Obligation::MPI_message_size() const {
  return 6 + _compressed_state.MPI_message_size() + Utils::MPI_message_size(_banned_actions);
}
