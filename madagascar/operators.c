...
// fprintf raw variants
void fprintfmalistraw(FILE* file,fmalist *);
void fprintfmaraw(FILE* file,fma *f) {
  switch(f->t) {
  case patom: fprintatomiraw(file,f->a); break;
  case natom: fprintf(file,"-"); fprintatomiraw(file,f->a); fprintf(file,""); break;
  case conj:
    fprintf(file,"(and ");
    fprintfmalistraw(file,f->juncts);
    fprintf(file,")");
    break;
  case disj:
    fprintf(file,"(or ");
    ...
    break;
  case TRUE:
    fprintf(file,"TRUE"); break;
  case FALSE:
    fprintf(file,"FALSE"); break;
  }
}

void fprintfmalistraw(FILE* f,fmalist *l) {
  if(l == NULL) return;
  fprintfmaraw(f,l->hd);
  if(l->tl != NULL) fprintf(f," ");
  fprintfmalistraw(f,l->tl);
}

void fprinteffraw(FILE *f, eff *e) {
  fma *c;
  int *ls;
  if(e == NULL) return;
  c = e->condition;
...
// fprintf variants
void fprintfmalist(FILE* file,fmalist *);
void fprintfma(FILE* file,fma *f) {
  switch(f->t) {
  case patom: fprintatomi(file,f->a); break;
  case natom: fprintf(file,"(not "); fprintatomi(file,f->a); fprintf(file,")"); break;
  case conj:
    fprintf(file,"(and ");
    fprintfmalist(file,f->juncts);
    fprintf(file,")");
    break;
  case disj:
    fprintf(file,"(or ");
...
void fprintfmalist(FILE* f,fmalist *l) {
  if(l == NULL) return;
  fprintfma(f,l->hd);
  if(l->tl != NULL) fprintf(f," ");
  fprintfmalist(f,l->tl);
}

void fprinteff(FILE *f, eff *e) {
  fma *c;
  int *ls;
  if(e == NULL) return;
  c = e->condition;
...
void fprintaction(FILE *f, int i) {
  int *l;
  /* Print operator name action(p1,...,pn) */
  l = actions[i].name;
  fprintf(f,"ACTION %i:%s(",i,symbol(*l));
  l = l + 1;
  while(*l != -1) {
    fprintf(f,"%s",symbol(*l));
    if(*(l+1) != -1) fprintf(f,",");
  ...
  fprintf(f,")      (COST %i)\n",actions[i].cost);
  /* Print precondition */
  fprintf(f,"action_pre ");
  fprintfmaraw(f,actions[i].precon);
  fprintf(f,"\n");
  /* Print effect */
  fprintf(f,"action_eff");
  fprinteffraw(f,&(actions[i].effects));
  fprintf(f,"\n");
}
...
void printaction(int i) {
  ...
  printf(")      (COST %i)\n",actions[i].cost);
  /* Print precondition */
  printf("OMIT action precon ");
  printfma(actions[i].precon);
  printf("\n");
  /* Print effect */
  printf("OMIT action eff ");
...
void eliminatestaticvariables() {
  ...
  */
  ...
  if(goalisdisjunctive) {
          printf("MAD    Goal: disjunctive\n");
          printf("ERROR, unknown if disjunctive goals can be processed, need to review encoding\n");
          exit(1);
  }
  else printf("MAD    Goal: conjunctive\n");
...
void mergecontras(FILE* f) {
  ...
  if(flagShowInput) {
    printf("OMIT TOTAL OF %i CONVERSES FOR %i VARIABLES.\n",cnt,nOfAtoms);
    ...
    fprintf(f,"UnusedAtoms:\n");
    for(i=0;i<nOfAtoms;i++) { if(mapping[i] != -1) { fprintf(f,"%d ",i); } }
    fprintf(f,"\n");
  }
