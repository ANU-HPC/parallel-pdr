...
void fprintvarraw(FILE* f,int v) {
  if(v & NEGtag) fprintf(f,"-");
  if(VARNEXT(v)) fprintf(f,"*");
  switch(VARTYPE(v)) {
  case AUXtag: fprintf(f,"AUX%i",VARINDEX(v)); break;
  case VARtag: fprintatomiraw(f,VARINDEX(v)); break;
  case OPtag: fprintf(f,"OP%i",VARINDEX(v)); break;
  default: fprintf(f,"(INCORRECT VAR TYPE %i",v); break;
  }
}

void fprintFfmalistraw(FILE* f,fmalist *,char *);
void fprintFfmaraw(FILE* file,fma *f) {
  switch(f->t) {
  case patom: fprintvarraw(file,f->a); break;
  case natom: fprintf(file,"-"); fprintvarraw(file,f->a); break;
  case conj:
    fprintf(file,"(");
    fprintFfmalistraw(file,f->juncts,"&");
    fprintf(file,")");
    break;
  case disj:
    fprintf(file,"(");
    ...
    fprintf(file,")");
    break;
  case TRUE: fprintf(file,"TRUE"); break;
  case FALSE: fprintf(file,"FALSE"); break;
  }
}

void fprintFfmalistraw(FILE* f,fmalist *l,char *sep) {
  if(l == NULL) return;
  fprintFfmaraw(f,l->hd);
  if(l->tl != NULL) fprintf(f,"%s",sep);
  fprintFfmalistraw(f,l->tl,sep);
}

void fprintvar(FILE* f,int v) {
  if(v & NEGtag) fprintf(f,"-");
  if(VARNEXT(v)) fprintf(f,"*");
  switch(VARTYPE(v)) {
  case AUXtag: fprintf(f,"AUX%i",VARINDEX(v)); break;
  case VARtag: fprintatomi(f,VARINDEX(v)); break;
  case OPtag: fprintf(f,"OP%i",VARINDEX(v)); break;
  default: fprintf(f,"(INCORRECT VAR TYPE %i",v); break;
  }
}

void fprintFfmalist(FILE* f,fmalist *,char *);
void fprintFfma(FILE* file,fma *f) {
  switch(f->t) {
  case patom: fprintvar(file,f->a); break;
  case natom: fprintf(file,"-"); fprintvar(file,f->a); break;
  case conj:
    fprintf(file,"(");
    fprintFfmalist(file,f->juncts,"&");
    fprintf(file,")");
    break;
  case disj:
    fprintf(file,"(");
...
void fprintFfmalist(FILE* f,fmalist *l,char *sep) {
  if(l == NULL) return;
  fprintFfma(f,l->hd);
  if(l->tl != NULL) fprintf(f,"%s",sep);
  fprintFfmalist(f,l->tl,sep);
}
...
#define RANDOMORDER
  ...
  for(i=0;i<nOfFmas;i++) {
    ...
    if(flagShowInput) {
      switch(fmas[i].cl) {
      case inittime: fprintf(f,"I: "); break;
      case goaltime: fprintf(f,"G: "); break;
      case transition: fprintf(f,"T: "); break;
      }
      fprintFfmaraw(f,fmas[i].f); fprintf(f,"\n");
    }
  ...
  if(flagOutputDIMACS) {
    printf("OMIT pretending to output plan to DIMACS, but not actually\n");
    //outputDIMACS();
  } else {
