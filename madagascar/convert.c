...
int convert_main(int argc,char **argv,const char* instance_location) {
  FILE* f = fopen(instance_location,"w");
  //FILE* f = stdout;
...
/* ******************************************************************** */
 ...
 if(planFrontend != 1) {
   ...
   if(debugOutput >= 1) printf("MAD   Grounding...\n");

   NEWgroundoperators();
  
   printf("MAD   Parser: %i ground actions and %i state variables\n",nOfActions,nOfAtoms);

   /* Not printing these, because these are the nonfinal non reduced actions
   if(flagShowInput) {
     for(i=0;i<nOfActions;i++) printaction(i);
   } */

   printf("OMIT first showing of all atoms, this may be ALL atoms, but these use a different indexing than they do at the end\n");
   /*
   if(flagShowInput) {
     printf("All atoms: ");
  ...
  if(!flagNoInvariants) printf("MAD     %.2f secs\n", time2real(time10ms() - TIMEinvariants));
  ...
  printf("MAD   Simplified: %i ground actions and %i state variables\n",nOfActions,nOfAtoms);
  ...
  */
  ...
  if(planSemantics == EStep) {
    ...
    maxsize = scc(f,nOfActions);
    printf("MAD     %.2f secs (max SCC size %i)\n",time2real(time10ms() - TIMEdisabling),maxsize);
  }
  ...
  // Everything is final at this point
  if(flagShowInput) {
    fprintf(f,"AllAtoms:\n");
    for(i=0;i<nOfAtoms;i++) {
      fprintf(f,"ATOM %i:",i);
      fprintatomi(f,i); fflush(stdout);
      fprintf(f,"\n");
    }
  }
  ...
  switch(sclass) {
  case STRIPS: fprintf(f,"Actions: STRIPS\n"); break;
  case Conjunctive: fprintf(f,"Actions: conjunctive\n"); break;
  case GeneralPDDL: fprintf(f,"Actions: general\n"); break;
  }

  if(flagShowInput) {
    for(i=0;i<nOfActions;i++) fprintaction(f,i);
  }
  ...
  printf("MAD   total time %.2f preprocess %.2f \n",
	 time2real(time10ms() - TIMEstart),
	 time2real(TIMEpreprocess - TIMEstart));

  givememorystatistics();

  printf("MAD   max. learned clause length %i\n",stats_longest_learned);

  if(flagOutputDIMACS == 0) {
    printf("MAD   t val conflicts decisions\n");
    i = 0;
    do {
