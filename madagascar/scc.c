...
sccs NNtraverse(FILE* f,int i,sccs ac) {
  int j,current;
  ...
  if((10*nodecounter / nOfActions) != (10*(nodecounter-1) / nOfActions)) {
    printf("MAD     %i\n",(nodecounter * 10 / nOfActions)*10); fflush(stdout);
  }
  ...
  if(NNlowlink[i] == NNdfnumber[i]) { /* Found an SCC */
    ...
    if(flagShowInput) {
      fprintf(f,"new SCC %i:",ac->NofEls);
      for(j=current;j<=NNptr;j++) fprintactionname(f,NNstack[j]); // printf(" %i",NNstack[j]);
      fprintf(f,"\n");
    } else {
      if(debugOutput > 0 && NNptr != current) printf("MAD    %i\n",NNptr-current+1);
    }
...
int scc(FILE* f,int nOfNodes) {
  ...
  printf("MAD Disabling graph %c:",'%');

  for(i=0;i<nOfNodes;i++)
    if(NNdfnumber[i] == -1) {
      ac = NNtraverse(f,i,ac);
    }
