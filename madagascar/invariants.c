...
void fprintlitlist(FILE* f, intset s) {
  int i;
  iITstart(s);
  while(iITnext(&i)) {
    if(i&1) fprintf(f," -");
    else fprintf(f," ");
    fprintatomiraw(f,feVAR(i));
  }
}

void showInvariants(FILE* f) {
  int i,cntpersistent;

  cntpersistent = 0;
  printf("OMIT Persistent literals\n");
  /*
  ...
  */
  ...
  for(i=0;i<nOfAtoms;i++) {
    fprintatomiraw(f,i); fprintf(f," OR:");
    fprintlitlist(f,twolits[fePLIT(i)]);
    fprintf(f,"\n");
    
    fprintf(f,"-"); fprintatomiraw(f,i); fprintf(f," OR:");
    fprintlitlist(f,twolits[feNLIT(i)]);
    fprintf(f,"\n");
  }
}
...
void computeinvariants() {
  ...
  //if(flagShowInput) showInvariants(f);
}
