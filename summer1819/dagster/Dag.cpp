...
Dag::Dag(const char *fname) {
  ...
  while ((c = getc(f)) != EOF) {
    if (isdigit(c)) {
      ungetc(c, f);
      if ((fscanf(f, "%u->%u\n", &from, &to) != 2) || (from >= no_nodes) || (to >= no_nodes) || (getc(f) != ':'))
...
void Dag::check_consistency() {
  // checks no duplicated clause assignments (NOTE: redundant check now)
  /*for (auto it = clause_indices_for_each_node.begin(); it != clause_indices_for_each_node.end(); it++) {
	  VLOG(2) << "about to check unique for " << it->size() << " clauses at node " << it-clause_indices_for_each_node.begin();
    if (!check_all_unique(*it))
      throw ConsistencyException(" Invalid Dag-file has repeated clauses for a node\n");
  }*/
  /*// checks no duplicate literal assignments (NOTE: redundant check now)
  for (auto it = forward_connection_literals.begin(); it != forward_connection_literals.end(); it++)
    for (auto it2 = (*it).begin(); it2 != (*it).end(); it2++) {
      if (!check_all_unique(*it2))
        throw ConsistencyException(" Invalid Dag-file has repeated litteral interface assignments!\n");
    }*/
