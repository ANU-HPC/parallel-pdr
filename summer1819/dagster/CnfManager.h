...
//** returns true if variable allready resolved straightforwardly
#define RESOLVED(lit)	(vars[VAR(lit)].value == SIGN(NEG(lit)))
//** returns true if variable allready resolved oppositely
...
struct Variable{
	...
	bool mark2;			// used in detecting variable redundancy
