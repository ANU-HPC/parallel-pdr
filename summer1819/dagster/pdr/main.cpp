  ...
  */
  ...
  if (command_line_arguments.mode == 3) {
    ...
    if (world_rank == 0) { // enter the master loop if rank zero
      ...
      //enter master loop
      MPICommsInterface* comms = new MPICommsInterface(&mastercommunicator);
      LingelingMaster master = LingelingMaster(comms,command_line_arguments.ENUMERATE_SOLUTIONS,command_line_arguments.BREADTH_FIRST_NODE_ALLOCATIONS);
      ...
      assert(0);
      lingelingWorker->loop("BAD_FILE_CHANGE_THIS", map<int, vector<int>>(), map<int, vector<int>>()); // enter the lingelingWorker loop otherwise
      delete comms;
