...
int SatSolver::selectLiteral() {
  ...
  if (short_stopping)
    if (verifySolution())
      return 0;
  if (heuristic_rotation == suggestion_first) // The first thing to try is to select a variable using an SLS process.
    x = get_suggestion();
  if (x == 0)
    x = selectLiteral__conflict();
  if ((x == 0) && (heuristic_rotation == cdcl_first)) // Local search has not been tried/successful, and we attempted to choose a variable from a conflict clause.
    x = get_suggestion();
  if (x == 0) // USE VSIDS
...
// run the CDCL process, returning 0 if contradiction, 1 if SAT, or 2 if paused
int SatSolver::run() {
  ...
  for (int lit; (lit = selectLiteral());) { // pick decision literal
    ...
    if (literals_selected == command_line_arguments.sat_reporting_time) {
      return 2; // return back up the stack to check whether master has reassignment.
    }
...
bool SatSolver::verify_and_trim_Solution() {
  ...
  for (int i=0; i<cnf->cc; i++) {
    ...
    for (int j=0; j<cnf->cl[i]; j++) {
      ...
      if (SET(lit)) {
        satisfied = true;
        if ((min_satisfying_var==-1)||(VAR(lit)<min_satisfying_var))
          min_satisfying_var = VAR(lit);
      }
    }
...
// clear the solver and any other reset needed
