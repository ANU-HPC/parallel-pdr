...
float float_time(clock_t time) { return ((float)time) / CLOCKS_PER_SEC; }
...
vector<int> uncompress_state(const vector<int> &compressed_state) {
  ...
  if (proper_state)
    return inflate_compressed_to_full(compressed_state,
                                      subproblem_to_propositions[subproblem]);
  else
    return inflate_compressed_to_full(compressed_state,
                                      subproblem_to_actions[subproblem]);
}
...
string str(const Obligation &obl) {
  return "[(state " + state_string(obl.state) + ") (layer " +
         to_string(obl.layer) + ") (subproblem " + to_string(obl.subproblem) +
         ") (worker " + to_string(obl.worker) + ") (reduce " +
         to_string(obl.reduce) + ") (keep_states " +
         to_string(obl.keep_states) + ")]";
}
...
bool state_conforms_to_only_one_cliques(vector<int> state,
  ...
  const vector<vector<int>> &cliques =
      subproblem_to_only_one_cliques[subproblem];
  for (auto it = cliques.begin(); it != cliques.end(); it++) {
    vector<int> clique = *it;
    sort(clique.begin(), clique.end());
    vector<int> intersection;
    set_intersection(state.begin(), state.end(), clique.begin(), clique.end(),
...
void BUFFER_print_state_count() {
  cout << "S \\ L";
  for (int layer = 0;
       layer < PDR::BUFFER_layer_to_subproblem_to_state_count.size(); layer++) {
    cout << "\t" << layer;
  }
  cout << endl;
  ...
  for (int subproblem = 0; subproblem < PDR::num_subproblems; subproblem++) {
    cout << subproblem;
    for (int layer = 0;
         layer < PDR::BUFFER_layer_to_subproblem_to_state_count.size();
         layer++) {
      cout << "\t"
...
#if VERBOSE_STATE
  if (abs(i) < symbols.size() && abs(j) >= symbols.size())
    return true;
  else if (abs(i) >= symbols.size() && abs(j) < symbols.size())
    return false;
  else if (abs(i) >= symbols.size() && abs(j) >= symbols.size())
    return abs(i) < abs(j);
...
string pos_state_string(vector<int> state) {
  ...
  for (int i = 0; i < state.size(); i++) {
    if (state[i] > 0) {
      ret_val += "  \033[38;5;10m";
      if (state[i] < symbols.size())
        ret_val += symbols[state[i]];
      else if (state[i] - total_per_timestep < symbols.size())
        ret_val += "*" + symbols[state[i] - total_per_timestep];
      else
        ret_val += to_string(state[i]);
    }
    ret_val += "\033[0m";
...
string state_string(set<int> state) {
  return state_string(vector<int>(state.begin(), state.end()));
}
...
int tilde_lit(int lit, int amount) {
  assert(lit != 0);
  if (lit > 0)
    return lit + amount * total_per_timestep;
  else
    return lit - amount * total_per_timestep;
}
...
vector<int> get_clause_from_reason(const vector<int> &reason,
  ...
  for (int i = 0; i < subproblem_to_clause_validating_lits[subproblem].size();
       i++) {
...
vector<int> project_state_to_only_one_mutex_cliques(const vector<int> &state,
  ...
  for (auto it = cliques.begin(); it != cliques.end(); it++) {
    ...
    for (int i = 0; i < projected.size(); i++) {
      if (projected[i] > 0)
        intersection.push_back(projected[i]);
    }
...
void State_Actions::set_final_state(const vector<int> &state,
                                    const int in_subproblem) {
  final_state = state;
  subproblem = in_subproblem;
  assert(subproblem == PDR::num_subproblems - 1); // should be the last one
}

void State_Actions::add_state(const vector<int> &succ_state,
                              const vector<int> &prev_state,
                              const vector<int> &actions,
                              const int state_subproblem,
                              const int succ_state_layer) {
  // TODO consider loops and minimal length paths?
  // Only add if not seen before
  const vector<int> &compressed_prev_state =
      PDR::compress_state(prev_state, state_subproblem, true);
  const vector<int> &compressed_succ_state =
      PDR::compress_state(succ_state, state_subproblem, true);
  const vector<int> &compressed_actions =
      PDR::compress_state(actions, state_subproblem, false);

  assert(succ_state.size() == prev_state.size());
  if (data.find(compressed_succ_state) == data.end()) {
    data[compressed_succ_state] = tuple<vector<int>, vector<int>, int>(
        compressed_prev_state, compressed_actions, succ_state_layer);
  }
}
...
void State_Actions::write_empty_plan() {
  ...
  if (PDR::isolate_subproblems)
    plan_file.open(tmp_dir + "/partial_plan" +
                   to_string(PDR::isolate_subproblems_number));
  else
    plan_file.open(tmp_dir + "/plan");
  plan_file.close();
}

void State_Actions::print_write_path(const vector<int> &final_state,
                                     int subproblem) {
  if (PRINT_END_RESULT)
    cout << "WRITING PLAN" << endl;
  ofstream plan_file;
  if (PDR::isolate_subproblems)
    plan_file.open(tmp_dir + "/partial_plan" +
                   to_string(PDR::isolate_subproblems_number));
  ...
  vector<int> compressed_prev_state =
      compress_state(final_state, subproblem, true);
  vector<int> compressed_actions_executed;
  vector<vector<int>> backwards_plan;
  assert(subproblem ==
         num_subproblems -
             1); // assume the plan we care about is the last/special one
  const vector<int> compressed_projected_initial_state =
      compress_state(project_state_to_propositions(
                         initial_state, subproblem_to_propositions[subproblem]),
                     subproblem, true);
  do {
    assert(seen_compressed_states.find(compressed_prev_state) ==
           seen_compressed_states.end());
    seen_compressed_states.insert(compressed_prev_state);

    compressed_succ_state = compressed_prev_state;

    assert(data.find(compressed_succ_state) !=
           data.end()); // Assert the map access will work
  ...
  } while (compressed_prev_state !=
           compressed_projected_initial_state); // if the previous state is not
                                                // the initial state go again
  ...
  plan_file.close();
  if (PRINT_END_RESULT)
    cout << "FINISHED WRITING PLAN" << endl;
}

void State_Actions::print_write_path_final_state() {
  if ((final_state.size() == 0) && subproblem == -1)
    write_empty_plan();
  else
    print_write_path(final_state, subproblem);
}
...
void Layers::print_reasons_size() {
  ...
  for (int layer = 0; layer < reasons.size(); layer++) {
    ...
    for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
      ...
      cout << "  Subproblem: " << subproblem << " ["
           << reasons[layer][subproblem].size() << "] avg size: "
           << string_float(reason_size_sum / ((all.size() + 0.0000001)))
           << endl;
    }
  }
}
...
void Layers::print_reasons() {
  ...
  for (int layer = 0; layer < reasons.size(); layer++) {
    ...
    for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
      cout << "  Subproblem: " << subproblem << " ["
           << reasons[layer][subproblem].size() << "]" << endl;
      const set<vector<int>> &all = reasons[layer][subproblem].get_all();
      for (auto it = all.begin(); it != all.end(); it++) {
...
void Layers::check() {
  ...
  vector<vector<set<vector<int>>>> layer_subproblem_reasons =
      vector<vector<set<vector<int>>>>(reasons.size());
  assert(layer_subproblem_reasons.size() == reasons.size());
  for (int layer = 0; layer < reasons.size(); layer++) {
    layer_subproblem_reasons[layer] = vector<set<vector<int>>>(num_subproblems);
    for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
      layer_subproblem_reasons[layer][subproblem] =
          reasons[layer][subproblem].get_all();
    }
  }
  ...
  for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
    for (int layer = 0; layer < reasons.size(); layer++) {
      ...
      for (auto it = X.begin(); it != X.end(); it++) {
        ...
        for (int check_layer = layer + 1; check_layer < reasons.size();
             check_layer++) {
          // see if this reason has been repeated
          cout << "check layer " << check_layer << endl;
          cout << "layer subprob size " << layer_subproblem_reasons.size()
               << endl;
          ...
          for (auto itb =
                   layer_subproblem_reasons[check_layer][subproblem].begin();
               itb != layer_subproblem_reasons[check_layer][subproblem].end();
               itb++) {
            vector<int> comparison_reason = *itb;
            cout << "checking layers " << layer << " -> " << check_layer
                 << " : " << state_string(reason) << endl;
...
void Layers::add_reason(const vector<int> &reason, int layer, int subproblem) {
  ...
  // check if weaker than a clause in the current layer - then drop the clause
  // (should be rare?..)
  if (LOUD && layer == 0)
    cout << "layer 0, this should only be for goal lits!" << endl;
  if (LOUD)
    cout << "add_reason layer " << layer
         << " , reason: " << state_string(reason) << endl;
  // TODO check for subsumption both ways, along the way

  // check if stronger than a clause in the current layer  AND all previous
  // layers. if so, remove ALL the weaker ones

  // Can cheaply check if it is a weaker clause - and avoid adding. Hard to
  // check if a stronger clause So if it is a stronger clause, just add as if
  // new
  ...
  if (reason.size() == 1 && layer == 0) {
    // cout << "needed?" << endl;
    if (LOUD)
      cout << "special for goal lits, adding directly" << endl;
  ...
  if (LOUD)
    cout << "check if adding clause stronger" << endl;
  ...
  if (LOUD)
    cout << "check if adding clause weaker" << endl;
  // TODO inneficient - investigate

  // if this, or a stronger reason is already registered at this or a higher
  // layer, don't add
  for (int check_layer = layer; check_layer < reasons.size(); check_layer++) {
    if (reasons[check_layer][subproblem].contains_subset(reason))
      return;
  }

  if (LOUD)
    cout << "new clause so adding it directly" << endl;
...
set<vector<int>> Layers::get_reasons_not_in_next_layer(int layer,
                                                       int subproblem) {
  return reasons[layer][subproblem].get_all();
}

bool Layers::same_as_previous(int nextLayer, set<int> subproblems_to_check) {
  const int layer = nextLayer - 1; // the layer to test if there is nothing here
                                   // that is not in the next layer
  assert(layer >= 0);
  assert(layer < reasons.size());

  if (subproblems_to_check.find(PDR::num_subproblems - 1) !=
      subproblems_to_check.end()) {
    ...
    // if any of the ones to check are empty, then that subproblem is unsat, so
    // the whole problem is unsat
    for (auto it = subproblems_to_check.begin();
         it != subproblems_to_check.end(); it++) {
      cout << "checking (layer, subproblem) (" << layer << "," << *it << ")"
           << endl;
      if (reasons[layer][*it].size() == 0)
        return true;
    }
    return false;
  }
...
void Layers::push_nonfinal_up_to_layer(int desired_just_empty_layer) {
  ...
  // make sure THIS layer is empty, but the one directly under is not (unless it
  // is a completely empty subproblem) uses manual enumerate then push (as
  // opposed to some more direct efficient method)
  ...
  for (int subproblem = 0; subproblem < num_subproblems - 1; subproblem++) {
    ...
    for (int layer = 0; layer < reasons.size(); layer++) {
      if (reasons[layer][subproblem].size())
        ...
        if (empty_layer == INT_MIN)
          empty_layer = layer;
      }
    }
    if (empty_layer == INT_MIN)
      empty_layer = reasons.size();
    max_empty_layer = max(max_empty_layer, empty_layer);

    // with that first empty layer, work out the difference for how much to push
    // up
    ...
    if (difference) {
      ...
      for (int layer = empty_layer; layer >= 0; layer--) {
        ...
        const set<vector<int>> &all_reasons =
            reasons[layer][subproblem].get_all();
...
vector<int> convert_clause_for_steps(vector<int> clause, int steps) {
  ...
  for (auto it = clause.begin(); it != clause.end(); it++) {
    const int lit = *it;
    assert(abs(lit) - PDR::total_per_timestep - 1 <
           PDR::total_per_timestep); // check that the original lit was in
                                     // timestep 2
    if (lit > 0)
      ret_val.push_back(lit + (steps - 1) * PDR::total_per_timestep);
    else
      ret_val.push_back(lit - (steps - 1) * PDR::total_per_timestep);
  }
  return ret_val;
...
void Dagster_Layers::solver_add_reason(const vector<int> &reason, int layer,
                                       int subproblem) { // TODO rename
  if (LOUD)
    cout << "dagster layer solver_add_reason layer, subproblem: " << layer
         << " " << subproblem << endl;

  // see if this should be added to the actual solver
  if (project_state_to_propositions(reason,
                                    subproblem_to_propositions[subproblem])
          .size() < reason.size()) {
    if (reason.size() == 1 && layer == 0) {
      cout << "SPECIAL EXEMPTION FOR GOAL LITS" << endl;
      if (LOUD)
        cout << "NOT ACTUALLY ADDING REASON TO SINGLE SOLVER (AS IS A GOAL "
                "PROP AT THE WRONG SUBPROBLEM - will add when relevant): "
             << state_string(reason) << endl;
      return;
    } else {
      if (LOUD)
        cout << "NOT ACTUALLY ADDING REASON AS DOESN'T PROJECT TO PROPOSITIONS "
                "USED, TO SINGLE SOLVER: "
             << state_string(reason) << endl;
      return;

      assert(0); // TODO what?...
      cout << "giving special excemption to the last one TODO review this "
              "whole system"
           << endl;
      // if (subproblem+1 != num_subproblems) assert(0); // this is baed on the
      // subproblem we happen to be in - problematic for adding a permament
      // clause
    }
  ...
  for (int worker = 1; worker < num_workers + 1; worker++) {
    while (worker_upper_layer_dagster_clauses_to_add[worker].size() <= layer) {
      worker_upper_layer_dagster_clauses_to_add[worker].push_back(
          vector<vector<int>>()); // just make sure it is full
    }
...
Single_Layers::~Single_Layers() {
  for (int i = 0; i < layer_to_steps_solvers.size(); i++) {
    for (int j = 1; j < layer_to_steps_solvers[i].size(); j++)
      delete layer_to_steps_solvers[i][j];
  }
}
...
void Single_Layers::make_layer_exist(int layer) {
  if (baseLingelings.size() == 0) {
    ...
    for (int steps = 1; steps < MAX_SOLVER_STEPS + 1; steps++) {
      ...
      baseLingelings.push_back(new Lingeling(
          (tmp_dir + "/tmp_regular_" + to_string(steps) + ".cnf").c_str()));
      if (use_OOC) {
        for (auto it = ooc_clauses_for_every_new_solver.begin();
             it != ooc_clauses_for_every_new_solver.end(); it++)
          baseLingelings[steps]->add_clause(*it);
      }
  ...
  while (reasons.size() < layer + 1) {
    ...
    layer_to_steps_solvers[new_layer_number].push_back(
        NULL); // as 1 indexed, for number of steps
    for (int steps = 1; steps < MAX_SOLVER_STEPS + 1; steps++) {
...
// upper layer to add to, this function adds to all less also
void Single_Layers::solver_add_reason(const vector<int> &reason,
                                      const int layer, const int subproblem) {
  if (LOUD)
    cout << "single layer solver_add_reason" << endl;
  ...
  for (auto it = clause.begin(); it != clause.end(); it++) {
    if (activation_vars.find(abs(*it)) == activation_vars.end())
      NAL_clause.push_back(*it);
  }
  ...
  for (int i = 0; i < layer + 1; i++) {
    if (LOUD)
      cout << "ACTUALLY ADDING CLAUSE TO SOLVER LAYER " << i << " : "
           << state_string(clause) << endl;
    if (LOUD)
      cout << "solver" << i << " add clause" << state_string(clause) << endl;

#if MULTI_STEP_INTERLEAVED
    // Interleaved
    for (int starting_higher_layer = 0;
         starting_higher_layer < MAX_SOLVER_STEPS; starting_higher_layer++) {
      const int max_steps =
          min(i + starting_higher_layer + 1, MAX_SOLVER_STEPS);
      ...
      for (int steps = min_steps; steps <= max_steps; steps++) {
        ...
        layer_to_steps_solvers[i + starting_higher_layer][steps]->add_clause(
            clause_to_add);
      }
    }
#else
    // Macros
    for (int steps = 1; steps < MAX_SOLVER_STEPS + 1; steps++) {
      layer_to_steps_solvers[i][steps]->add_clause(
          convert_clause_for_steps(clause, steps));
    }
#endif
...
project_to_state_actions(const vector<int> &assignment, const int subproblem,
  ...
  for (int timestep = 0; timestep < steps_used + 1; timestep++) {
    ...
    const int upper_action_bound =
        num_actions + timestep * total_per_timestep + 1;
    const int lower_proposition_bound =
        num_actions + timestep * total_per_timestep + 1;
    ...
    for (int action_var = lower_action_bound; action_var < upper_action_bound;
         action_var++) {
      actions_executed.push_back(
          tilde_lit(assignment[action_var - 1], -timestep));
      assert(abs(assignment[action_var - 1]) == action_var);
      DEBUG_largest_var = max(DEBUG_largest_var, action_var);
    }

    for (int proposition_var = lower_proposition_bound;
         proposition_var < upper_proposition_bound; proposition_var++) {
      succ_state.push_back(
          tilde_lit(assignment[proposition_var - 1], -timestep));
    ...
    assert(actions_executed ==
           project_state_to_propositions(actions_executed, PDR::actions));
    assert(succ_state ==
           project_state_to_propositions(
               succ_state,
               PDR::subproblem_to_propositions[0])); // TODO fix for subproblems
  ...
  return pair<vector<vector<int>>, vector<vector<int>>>(
      succ_state_sequence, actions_executed_sequence);
}
...
void read_extra_settings(string extra_settings_filename) {
  ...
  // read extra settings - the is mainly for testing, so new feastures can be
  // quickly added and removed
  const int total_expected = 10;
  set<string> ignore_keys;
  ignore_keys.insert("activation_literals");

  // note ignoring keys that do matter, but they are given by a different
  // channel for now
  ...
  while (getline(extra_settings_file, line)) {
    ...
    if (ignore_keys.find(key) != ignore_keys.end())
      continue;
      ...
      if (val_int == 1)
        complete_nonfinal = true;
      else if (val_int == 0)
        complete_nonfinal = false;
      else
        assert(0);
      ...
      if (val_int == 1)
        isolate_subproblems = true;
      else if (val_int == 0)
        isolate_subproblems = false;
      else
        assert(0);
    } else {
  ...
  if (complete_nonfinal && project_last) {
    assert(0); // Can't handle projecting AND complete nonfinal - will try do
               // them together, and not do it well
  }
...
void read_problem_input() {
  ...
  subproblem_to_clause_validating_lits =
      parser.subproblem_to_clause_validating_lits;
  ...
  for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
    ...
    for (int clique = 0;
         clique < subproblem_to_only_one_cliques[subproblem].size(); clique++) {
      vector<int> clause_zero =
          subproblem_to_only_one_cliques[subproblem][clique];
      ...
      for (int i = 0;
           i < subproblem_to_clause_validating_lits[subproblem].size(); i++) {
        clause_zero.push_back(
            subproblem_to_clause_validating_lits[subproblem][i]);
      }

      for (int i = 0;
           i < subproblem_to_clause_validating_lits[subproblem].size(); i++) {
        clause_one.push_back(
            subproblem_to_clause_validating_lits[subproblem][i]);
      }
  ...
  for (int subproblem = 0; subproblem < PDR::num_subproblems; subproblem++) {
    activation_vars.insert(
        abs(PDR::subproblem_to_clause_validating_lits[subproblem][0]));
  }
#endif
}
...
void read_problem_input_isolate_subproblems() {
  ...
  num_subproblems = 1;
  subproblem_to_propositions[0] =
      parser.subproblem_to_propositions[PDR::isolate_subproblems_number];
  subproblem_to_actions[0] =
      parser.subproblem_to_actions[PDR::isolate_subproblems_number];
  subproblem_to_clause_validating_lits[0] =
      parser.subproblem_to_clause_validating_lits
          [PDR::isolate_subproblems_number];
  subproblem_to_assumptions[0] =
      parser.subproblem_to_assumptions[PDR::isolate_subproblems_number];
  subproblem_to_only_one_cliques[0] =
      parser.subproblem_to_only_one_cliques[PDR::isolate_subproblems_number];
  goal_condition =
      parser.subproblem_to_isolate_goal[isolate_subproblems_number];
  initial_state = project_state_to_propositions(parser.initial_state,
                                                subproblem_to_propositions[0]);
...
void dagster_initialize_to_send_clauses(
  ...
  for (auto it = workers_to_compile_clauses_for.begin();
       it != workers_to_compile_clauses_for.end(); it++) {
    const int worker = *it;
    assert(worker);
    for (int layer = 0;
         layer < worker_upper_layer_dagster_clauses_to_add[worker].size();
         layer++) {
      const vector<vector<int>> &clauses =
          worker_upper_layer_dagster_clauses_to_add[worker][layer];
      if (clauses.size()) {
        if (A)
          cout << "preparing an extra clauses message for worker " << worker
               << " with clauses sizes: ";
...
// TODO neatness rename the layer for state layer, and succ state layer for
// clarity
vector<int> single_get_successor(const vector<int> &state, int layer,
                                 int subproblem, bool record) {
  // TODO giving memo result in the function means unsat cores will be wrong for
  // subsequent reason call
  if (LOUD)
    single_layers.print_reasons();
  if (LOUD)
    cout << "state: " << state_string(state) << endl;

  if (LOUD)
    cout << "single_get_successor for layer " << layer << endl;
  assert(single_layers.layer_to_steps_solvers.size() > layer - 1);
  timing.lingeling_start(subproblem);
  // cout << "solver" << layer-1 << " all assumptions " << state_string(state)
  // << " " << state_string(subproblem_to_assumptions[subproblem]) << endl;
  bool success1;
#if TEST_NAL
  bool NAL_success1;
#endif
  if (PDR::use_OOC) {
    const vector<int> &projected_state =
        project_state_to_only_one_mutex_cliques(state, subproblem);
    parallel_timer.start();
    success1 =
        single_layers.layer_to_steps_solvers[layer - 1][PDR::MS_steps_used]
            ->solve_unmentioned_assumptions(
                projected_state, subproblem_to_assumptions[subproblem]);
#if TEST_NAL
    NAL_success1 =
        single_layers.NAL_solvers[layer - 1]->solve_unmentioned_assumptions(
            projected_state, subproblem_to_assumptions[subproblem]);
#endif
    ...
    success1 =
        single_layers.layer_to_steps_solvers[layer - 1][PDR::MS_steps_used]
            ->solve_unmentioned_assumptions(
                state, subproblem_to_assumptions[subproblem]);
#if TEST_NAL
    NAL_success1 =
        single_layers.NAL_solvers[layer - 1]->solve_unmentioned_assumptions(
            state, subproblem_to_assumptions[subproblem]);
  ...
  if (success1) {
    ...
    pair<vector<vector<int>>, vector<vector<int>>> x =
        project_to_state_actions(assignments, subproblem, PDR::MS_steps_used);
    vector<vector<int>> succ_state_sequence = get<0>(x);
    vector<vector<int>> actions_executed_sequence = get<1>(x);
    if (PDR::storing_actions && record &&
        subproblem == PDR::num_subproblems - 1)
      assert(succ_state_sequence[0] == state); // broken on subproblems?
    vector<int> prev_state = state;
    for (int timestep = 0; timestep < PDR::MS_steps_used; timestep++) {
      if (LOUD)
        cout << "single found a successor state, used actions: "
             << state_string(actions_executed_sequence[timestep]) << endl;
      if (PDR::storing_actions && record &&
          subproblem == PDR::num_subproblems - 1)
        state_actions.add_state(
            succ_state_sequence[timestep + 1], succ_state_sequence[timestep],
            actions_executed_sequence[timestep], subproblem, layer + 1);
...
bool single_has_successor(const vector<int> &state, int layer, int subproblem) {
  if (LOUD)
    cout << "single_has_successor for layer " << layer
         << " subproblem: " << subproblem << endl;
  assert(single_layers.layer_to_steps_solvers.size() > layer - 1);

  // check to make sure this is being used as thought
  if (PDR::use_OOC)
    assert(state.size() ==
           project_state_to_only_one_mutex_cliques(state, subproblem).size());
...
vector<int> single_get_reason_just_used(const vector<int> &state, int layer,
                                        int subproblem) {
  return single_layers.layer_to_steps_solvers[layer - 1][PDR::MS_steps_used]
      ->used_assumptions();
}
...
vector<int> single_get_reason(const vector<int> &state, int layer,
  ...
  vector<int> running_reason =
      single_layers.layer_to_steps_solvers[layer - 1][PDR::MS_steps_used]
          ->used_assumptions();
  if (S)
    cout << "in layer " << layer << endl;
  if (S)
    cout << "subproblem " << subproblem << endl;
  if (S)
    cout << "In reduce, starting candidate [ " << running_reason.size() << " / "
         << subproblem_to_propositions[subproblem].size() << " ] "
         << state_string(running_reason) << endl;
  ...
  for (int i = 0; i < propositions_to_exclude.size(); i++) {
    ...
    for (int j = 0; j < running_reason.size(); j++) {
      if (abs(running_reason[j]) != proposition_to_exclude)
        possible_reason.push_back(running_reason[j]);
    }
      ...
      if (!single_has_successor(possible_reason, layer, subproblem)) {
        running_reason =
            single_layers.layer_to_steps_solvers[layer - 1][PDR::MS_steps_used]
                ->used_assumptions();
        if (S)
          cout << "In reduce, mid-way candidate [ " << running_reason.size()
               << " / " << subproblem_to_propositions[subproblem].size()
               << " ] " << state_string(running_reason) << endl;
      }
    }
...
void print_dagster_stats() {
  for (int subproblem = 0; subproblem < PDR::num_subproblems; subproblem++) {
    cout << "dagster_sat_count " << subproblem << " : "
         << dagster_sat_counts[subproblem] << endl;
  }
  for (int subproblem = 0; subproblem < PDR::num_subproblems; subproblem++) {
    cout << "dagster_unsat_count " << subproblem << " : "
         << dagster_unsat_counts[subproblem] << endl;
  }
  cout << "dagster_call_count: " << PDR::dagster_call_count << endl;
  cout << "dagster_total_time: " << string_float(float_time(dagster_total_time))
       << endl;

  cout << "simulated_parallel_total_time: "
       << string_float(float_time(total_parallel_time_spent)) << endl;
}
...
void dagster_start(int argc, char **argv) {
  ...
  global_init(argc, argv, PDR::tmp_dir, subproblem_to_propositions,
              subproblem_to_assumptions);
...
void dagster_stop() { shutdown(); }

void add_reason(const vector<int> &reason, int layer, int subproblem) {
  if (PDR::runtime_dagster)
    return dagster_layers.add_reason(reason, layer, subproblem);
  else
    return single_layers.add_reason(reason, layer, subproblem);
}

void make_layer_exist(int layer) {
  if (PDR::runtime_dagster)
    dagster_layers.make_layer_exist(layer);
  else
    single_layers.make_layer_exist(layer);
}

set<vector<int>> get_reasons_not_in_next_layer(int layer, int scc_node) {
  if (PDR::runtime_dagster)
    return dagster_layers.get_reasons_not_in_next_layer(layer, scc_node);
  else
    return single_layers.get_reasons_not_in_next_layer(layer, scc_node);
}

bool same_as_previous(int layer, set<int> layers_to_check) {
  if (PDR::runtime_dagster)
    return dagster_layers.same_as_previous(layer, layers_to_check);
  else
    return single_layers.same_as_previous(layer, layers_to_check);
}

int get_first_empty_layer() {
  if (PDR::runtime_dagster)
    return dagster_layers.get_first_empty_layer();
  else
    return single_layers.get_first_empty_layer();
}

void push_nonfinal_up_to_layer(int just_empty_layer) {
  if (PDR::runtime_dagster)
    return dagster_layers.push_nonfinal_up_to_layer(just_empty_layer);
  else
    return single_layers.push_nonfinal_up_to_layer(just_empty_layer);
}
