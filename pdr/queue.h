...
class Queue_Entry {
public:
  Queue_Entry(const vector<int> &in_compressed_state,
              const int in_heuristic_cost,
              const unsigned long long int in_adding_timestamp);
...
public:
  PDR_Queue(int in_num_subproblems, int *in_obligation_rescheduling_upper_layer,
  ...
  void remove_states_matching_reason_layer_subproblem(const vector<int> &reason,
                                                      const int max_layer,
  ...
  tuple<vector<int>, int, int> get_next_state_layer_subproblem(int layer,
                                                               int subproblem);
  ...
  tuple<vector<int>, int, int> contemplated_monolyth =
      tuple<vector<int>, int, int>(vector<int>(), 0, 0);
...
private:
  ...
  vector<vector<set<Queue_Entry, queue_entry_comp_struct> *>>
      data; // subproblem to layer to deque of compressed states
...
public:
  PDR_Portfolio_Queue(int in_num_subproblems,
  ...
  pop(const set<int> &free_workers);
