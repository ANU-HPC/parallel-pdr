...
// PDR_Queue 
PDR_Queue::PDR_Queue(int in_num_subproblems, int* in_obligation_rescheduling_upper_layer, bool in_performing_obligation_rescheduling, map<int, vector<int>> in_subproblem_to_propositions) {
...
// PDR_Portfolio_Queue
PDR_Portfolio_Queue::PDR_Portfolio_Queue(int in_num_subproblems, int* in_obligation_rescheduling_upper_layer, bool in_performing_obligation_rescheduling, int in_num_workers, map<int, vector<int>> in_subproblem_to_propositions) {
  num_workers = in_num_workers;
  worker_queues.push_back(NULL);

  for (int i=0; i<num_workers; i++) {
    worker_queues.push_back(new PDR_Queue(in_num_subproblems, in_obligation_rescheduling_upper_layer, in_performing_obligation_rescheduling, in_subproblem_to_propositions));
  }
}
