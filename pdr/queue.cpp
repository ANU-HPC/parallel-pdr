...
public:
  compressed_not_matches_reason(const vector<int> &reason) : reason(reason) {}
  ...
  bool operator()(const vector<int> &compressed_state) const {
    ...
    assert(
        compressed_state[0] ==
        0); // This asserts the subproblem is 0. This is not necessary, but it
            // is included here as a note/reminder, that this matches operator
            // is only correct if the the subproblem of the 2 things are the
            // same. There is a case where a reason mentions a variable outside
            // of the state, this algorithm will read it as the state saying
            // that lit is negative, as opposed to just not relevant.

    assert(is_sorted(compressed_state_current, compressed_state_end));
    assert(is_abs_sorted(reason));

    while (compressed_state_current != compressed_state_end &&
           reason_current != reason_end) {
      if (*compressed_state_current ==
          *reason_current) { // matching vars AND lits
        compressed_state_current++;
        reason_current++;
      } else if (*compressed_state_current ==
                 -*reason_current) { // opposing lits
        assert(reason_current != reason.end());
        return true;
      } else if (abs(*compressed_state_current) > abs(*reason_current)) {
        if ((*reason_current) > 0)
          return true; // reason lit was positive, but the cstate skipped it
        else
          reason_current++;
      } else {
...
Queue_Entry::Queue_Entry(const vector<int> &in_compressed_state,
                         const int in_heuristic_cost,
                         const unsigned long long int in_timestamp) {
...
// PDR_Queue
PDR_Queue::PDR_Queue(int in_num_subproblems,
                     int *in_obligation_rescheduling_upper_layer,
                     bool in_performing_obligation_rescheduling,
                     map<int, vector<int>> in_subproblem_to_propositions) {
...
void PDR_Queue::
    helper_remove_states_matching_reason_include_layer_subproblem_specific_layer(
        set<Queue_Entry, queue_entry_comp_struct> *specific_queue,
        const vector<int> &reason, const int layer, const int subproblem) {
  if (specific_queue->size() == 0)
    return;
  ...
  while (it != specific_queue->end()) {
    if (!function_keep(it->compressed_state)) {
      // If not keeping, remove it, but first check for obligation rescheduling,
      // and remove the hash
      DEBUG_num_removed++;

      // remove from hash
      pair<vector<int>, int> thing1 =
          pair<vector<int>, int>(it->compressed_state, layer);
...
void PDR_Queue::remove_states_matching_reason_layer_subproblem(
    const vector<int> &reason, const int max_layer, const int subproblem) {
  for (int layer = 0; layer <= max_layer; layer++) {
    ...
    helper_remove_states_matching_reason_include_layer_subproblem_specific_layer(
        data[layer][subproblem], reason, layer, subproblem);
    ...
    const int final_subproblem = num_subproblems - 1;
    if (subproblem != final_subproblem) {
      helper_remove_states_matching_reason_include_layer_subproblem_specific_layer(
          data[layer][final_subproblem], reason, layer, subproblem);
...
void PDR_Queue::remove_subproblem(int subproblem) {
  ...
  for (int layer = 0; layer < data.size(); layer++) {
    if (data[layer][subproblem]->size()) {
      assert(available_layers_subproblems[layer].find(subproblem) !=
             available_layers_subproblems[layer].end());
      available_layers_subproblems[layer].erase(subproblem);
      if (available_layers_subproblems[layer].size() == 0) {
        available_layers.erase(layer);
      }
  ...
  if (active_subproblems.find(subproblem) != active_subproblems.end())
    active_subproblems.erase(subproblem);
}
...
void PDR_Queue::make_layer_exist(int layer) {
  while (data.size() <= layer) {
    ...
    for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
      set<Queue_Entry, queue_entry_comp_struct> *stack =
          new set<Queue_Entry, queue_entry_comp_struct>();
...
bool PDR_Queue::empty_for_subproblem(const int subproblem) {
  if (subproblem == num_subproblems - 1) {
    if (get<0>(contemplated_monolyth).size() != 0)
      return false;
  }
  for (int layer = 0; layer < data.size(); layer++) {
    if (data[layer][subproblem]->size())
...
bool PDR_Queue::empty() {
  return (available_layers.size() == 0) &&
         (get<0>(contemplated_monolyth).size() == 0);
}
...
PDR_Queue::get_next_state_layer_subproblem(int layer, int subproblem) {
  // checks
  assert(!empty());
  assert(available_layers_subproblems[layer].size());
  ...
  pair<vector<int>, int> thing1 =
      pair<vector<int>, int>(queue_entry.compressed_state, layer);
...
void PDR_Queue::a_top_projection_failed() {
  ...
  if (state.size()) {
    if (performing_obligation_rescheduling) {
      if (layer < *obligation_rescheduling_upper_layer) {
        assert(subproblem == num_subproblems - 1);
...
vector<tuple<vector<int>, int, int, bool, bool>>
PDR_Queue::pop(int num_states) {
  assert(!empty());
  assert(num_states);
  ...
  set<int> available_layers_copy =
      available_layers; // Set of all the layers with something in them.
  assert(available_layers_copy.size());
  // Note Charles had this loop split so "auto it" was on its own line. Don't
  // know why
  for (auto it = available_layers_copy.begin();
       it != available_layers_copy.end(); it++) {
    const int layer = *it;
    // This used to actually be half the remaining states, then would go back
    // and back. By removing the "/2" it makes it greedy
    ...
    for (int i = 0; i < num_wanted_states_now; i++) {
      if (available_layers.find(layer) != available_layers.end()) {
        // if (clean_is_available_layer(layer, back)) { // This was removed, I
        // believe because it was expensive
        const tuple<vector<int>, int, int> &X = get_next_state_layer(
            layer); // This has been broken into 2 lines, because there are
                    // inconsistent return types
        ret_val.push_back(tuple<vector<int>, int, int, bool, bool>(
            get<0>(X), get<1>(X), get<2>(X), true, true));
      } else
        break;
    }
    // Charles's way of doing this
    if (it == available_layers_copy.begin())
      break;
  }
...
int PDR_Queue::get_novelty_heuristic_cost(const vector<int> &compressed_state) {
  ...
  auto single_end =
      std::set_intersection(compressed_state.begin() + 2,
                            compressed_state.end(), novelty_single.begin(),
                            novelty_single.end(), single_intersection.begin());
  ...
  for (auto base_prop_it = compressed_state.begin() + 2;
       base_prop_it != compressed_state.end(); base_prop_it++) {
    // see if this base_prop has a novelty_pair entry
    if (novelty_pair.find(*base_prop_it) == novelty_pair.end())
      continue;
    ...
    auto pair_end = std::set_intersection(
        base_prop_it + 1, compressed_state.end(), other_props.begin(),
        other_props.end(), pair_intersection.begin());

    // If there was some intersection, mark that a pair has been found, and
    // remove the entries from novelty_pair
...
#if USE_HEURISTIC
  return get_novelty_heuristic_cost(compressed_state);
#else
  return 1;
#endif
}
...
void PDR_Queue::push(const vector<int> &state, int layer, int subproblem) {
  ...
  const vector<int> &compressed_state =
      PDR::compress_state(state, subproblem, true);
  const int heuristic_cost = get_heuristic_cost(compressed_state);
  const unsigned long long int timestamp = next_timestamp;
  next_timestamp++;
  push(Queue_Entry(compressed_state, heuristic_cost, timestamp), layer,
       subproblem);
}
...
void PDR_Queue::push(Queue_Entry queue_entry, int layer, int subproblem) {
  ...
  pair<vector<int>, int> thing1 = pair<vector<int>, int>(
      queue_entry.compressed_state, layer); // C.G. added 'hash blocker' here
  pair<pair<vector<int>, int>, int> thing2 =
      pair<pair<vector<int>, int>, int>(thing1, subproblem);
  if (hash.find(thing2) != hash.end())
    return;
  hash.insert(thing2);
  ...
  assert(debug_removed_subproblems.find(subproblem) ==
         debug_removed_subproblems.end());
  ...
  for (int layer = 0; layer <= max_layer_ever; layer++) {
    ...
    for (int subproblem = 0; subproblem < num_subproblems; subproblem++) {
      cout << "  subproblem " << subproblem << ": "
           << data[layer][subproblem]->size() << endl;
    }
  }
}
...
PDR_Portfolio_Queue::PDR_Portfolio_Queue(
    int in_num_subproblems, int *in_obligation_rescheduling_upper_layer,
    bool in_performing_obligation_rescheduling, int in_num_workers,
    map<int, vector<int>> in_subproblem_to_propositions) {
  num_workers = in_num_workers;
  worker_queues.push_back(NULL);

  for (int i = 0; i < num_workers; i++) {
    worker_queues.push_back(new PDR_Queue(
        in_num_subproblems, in_obligation_rescheduling_upper_layer,
        in_performing_obligation_rescheduling, in_subproblem_to_propositions));
  }
}
...
void PDR_Portfolio_Queue::push(const vector<int> &state, int layer,
                               int subproblem, int worker) {
  worker_queues[worker]->push(state, layer, subproblem);
  assert(!worker_queues[worker]->empty());
}

void PDR_Portfolio_Queue::remove_states_matching_reason_layer_subproblem(
    const vector<int> &reason, const int max_layer, const int subproblem) {
  // cout << "WARN NOT REMOVING MATCHING (and clause pushing)" << endl;
  for (int worker = 1; worker < num_workers + 1; worker++) {
    worker_queues[worker]->remove_states_matching_reason_layer_subproblem(
        reason, max_layer, subproblem);
  }
}
...
vector<tuple<vector<int>, int, int, int, bool, bool>>
PDR_Portfolio_Queue::pop(const set<int> &free_workers) {
  ...
  for (auto it = free_workers.begin(); it != free_workers.end(); it++) {
    ...
    if (!worker_queues[worker]->empty()) {
      ...
      const vector<tuple<vector<int>, int, int, bool, bool>> &queue_elements =
          worker_queues[worker]->pop(1);
      assert(queue_elements.size() == 1);

      const tuple<vector<int>, int, int, bool, bool> &X =
          *queue_elements.begin();
      ...
      ret_val.push_back(tuple<vector<int>, int, int, int, bool, bool>(
          s, i, subproblem, worker, reduce, keep_state));
    }
  }
  return ret_val;
}
...
bool PDR_Portfolio_Queue::empty_for_subproblem(int subproblem) {
  // say empty iff all empty
  for (int worker = 1; worker < num_workers + 1; worker++) {
    if (!worker_queues[worker]->empty_for_subproblem(subproblem))
      return false;
  }
  return true;
}

void PDR_Portfolio_Queue::remove_subproblem(int subproblem) { exit(3); }

void PDR_Portfolio_Queue::a_top_projection_failed() { exit(4); }
