...
bool abs_comp(int i, int j) { return abs(i) < abs(j); }

bool is_abs_sorted(const vector<int> &state) {
  return is_sorted(state.begin(), state.end(), abs_comp);
}

Set_Trie::Set_Trie() { root_node = new Set_Trie_Node; }
...
bool Set_Trie::contains_remove(const vector<int> input) {
  ...
  // Needs to be a collection of unique elements
  vector<Set_Trie_Node *>
      path; // the path taken, to backtrack and erase if necessary
  assert(is_abs_sorted(input));
  for (int i = 1; i < input.size(); i++)
    assert(input[i - 1] != input[i]);
  ...
  if (node->terminal) {
    ...
    if (node->has_no_child()) { // if this terminal node is NOT in the path of
                                // another
      assert(path.size() >= 2); // root node, and this terminal node
      for (int i = path.size() - 2; i >= 0;
           i--) { // NOTE the -2, start at the parent, TODO minor efficiency
                  // with paths
        bool parent_has_sole_child_and_is_not_terminal =
            path[i]->has_sole_child() &&
            (!path[i]->terminal); // TODO neaten this logic
        // remove child
        delete path[i + 1];

        // remove child from map, TODO efficiency check if deleting the entire
        // parent node anyway
        assert(
            path[i]->has_child(input[i])); // CAREFUL different index references
        path[i]->children.erase(input[i]);
        if (!parent_has_sole_child_and_is_not_terminal) { // parent has many
                                                          // children, so stop
                                                          // here
          break;
        }
      }
    } else { // node is on the path to other sets, so keep it but mark it
             // non-terminal
      node->terminal = false;
    }
    count--;
...
bool Set_Trie::contains_subset(vector<int> input) {
  ...
  for (int i = 1; i < input_ptr->size(); i++)
    assert((*input_ptr)[i - 1] != (*input_ptr)[i]);

  map<pair<Set_Trie_Node *, int>, bool> *memo =
      new map<pair<Set_Trie_Node *, int>, bool>;
  bool return_value = contains_subset_node(memo, root_node, input_ptr, 0);
  delete memo;
  return return_value;
}

bool Set_Trie::contains_subset_node(map<pair<Set_Trie_Node *, int>, bool> *memo,
                                    Set_Trie_Node *node, vector<int> *input,
                                    int index) {
  ...
  if (node->has_child((*input)[index])) {
    node = node->children[(
        *input)[index]]; // TODO check reassigning node like this is ok?
...
int Set_Trie::size() { return count; }

void Set_Trie::get_all_recursive(set<vector<int>> *all, Set_Trie_Node *node,
                                 vector<int> *current) {
  if (node->terminal)
    all->insert(*current);
