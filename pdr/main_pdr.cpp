  ...
  void dagster_compute(bool use_sync_solve) {
    ...
    for (auto it=assignments_messages.begin(); it!=assignments_messages.end(); it++) {
      ...
      */
      ...
      const int DEBUG_dagster_num_steps = 1; // As the parallel version is only set up to handle 1 step
      tie(states, actions_collections) = PDR::project_to_state_actions(assignment, original_obl.subproblem, DEBUG_dagster_num_steps);
      ...
      if (PDR::storing_actions && original_obl.keep_states && original_obl.subproblem == PDR::num_subproblems-1) PDR::state_actions.add_state(succ_obl.state, original_obl.state, actions_executed, original_obl.subproblem, original_obl.layer);
...
bool push_check_unsat(Parallel_Buffer* buffer_ptr, QUEUE_TYPE* queue_ptr, const int layer_just_completed, const set<int>& subproblems_to_push, const set<int>& subproblems_to_check);

bool handle_nogoods_from_buffer_unsat(Parallel_Buffer* buffer_ptr, QUEUE_TYPE* queue_ptr, const int* layers, const set<int>* completed_subproblems_ptr);

bool handle_successor_states_from_buffer_sat(Parallel_Buffer* buffer_ptr, QUEUE_TYPE* queue_ptr, const int layer, set<int>* completed_subproblems_ptr, set<int>* newly_completed_subproblems_ptr);

bool populate_buffer_from_queue(Parallel_Buffer* buffer_ptr, QUEUE_TYPE* queue_ptr, const bool final_subproblem);
...
bool parallel_pdr() {
  ...
  PDR::make_layer_exist(0);
#if PORTFOLIO_QUEUE
  PDR::PDR_Portfolio_Queue queue = QUEUE_TYPE(PDR::num_subproblems, layers + PDR::num_subproblems-1, PDR::obligation_rescheduling, PDR::num_workers, PDR::subproblem_to_propositions);
#else
  PDR_Queue queue = QUEUE_TYPE(PDR::num_subproblems, layers + PDR::num_subproblems-1, PDR::obligation_rescheduling, PDR::subproblem_to_propositions);
...
#else
bool populate_buffer_from_queue(Parallel_Buffer* buffer_ptr, PDR_Queue* queue_ptr, const bool final_subproblem) {
  //cout << "final_subproblem? " << final_subproblem << endl;
  if (!queue_ptr->empty()) {
    ...
    // May be empty
    vector<tuple<vector<int>, int, int, bool, bool>> considered_states = queue_ptr->pop(free_workers.size());
  ...
  for (int i=1; i <= layer_just_completed+1; i++) {
    ...
    if (buffer_to_process) {
      ...
      while (unprogressable_states_with_info.size()) {
        ...
        //cout << "found nogood " << PDR::state_string(unprogressable_state) << endl;
        queue_ptr->remove_states_matching_reason_layer_subproblem(original_obl.state, original_obl.layer, original_obl.subproblem);
