...
/* Number of deterministic components of non-deterministic actions */

int nOfDActions;

typedef struct _Daction {
  int *name;
  fma *precon;
  eff *effects;
} Daction;
...
void preprocess() {
  ...
  /* Split non-deterministic actions to deterministic ones. */

  nOfDActions = 0;
  for(i=0;i<nOfActions;i++) {
    nOfDActions += actions[i].nOfEffects;
  }

  Dactions = (Daction *)malloc(nOfDActions * sizeof(Daction));
  DtoND = (Daction *)malloc(nOfDActions * sizeof(Daction));

  nOfDActions = 0;
  for(i=0;i<nOfActions;i++) {
    for(j=0;j<actions[i].nOfEffects;j++) {
      Dactions[nOfDActions].name = actions[i].name;
      Dactions[nOfDActions].precon = actions[i].precon;
      Dactions[nOfDActions].effects = actions[i].effects[j];
      DtoND[nOfDActions] = i;
      nOfDActions += 1;
    }
  }
  ...
  prepros = (llist **)statmalloc(401,nOfDActions * sizeof(llist *));
  preconlits = IScreate(1000);

  for(i=0;i<nOfDActions;i++) {

    e = Dactions[i].effects;
...
#ifdef DEBUG
    printf("Preprocessing for action %i:",i); fflush(stdout);
    printaction(DtoND[i]); fflush(stdout);
#endif

    ISmakeempty(preconlits);

    guaranteedtrue(Dactions[i].precon,preconlits);
    ...
    while(e != NULL) { /* Go through the conditional effects */
      ...
      addeffectlitsL(e->effectlits,e,Dactions[i].effects,prepros[i][j].after);
#ifdef DEBUG
...
void preprocess2() {
  ...
  alleffectlits = (int **)statmalloc(499,nOfDActions * sizeof(int *));
  for(i=0;i<nOfDActions;i++) {
    sz = 1 + nofeffectlits(Dactions[i].effects);
...
void computeinvariants() {
  ...
  while(change) {
    ...
    for(i=0;i<nOfDActions;i++) {
      ...
      if(!truep(Dactions[i].precon)) continue; /* Not applicable (yet) */
	...
	if(truep(e->condition)) {
	  ...
	  while(*il != -1) {
		...
		removefalsified(Dactions[i].effects,trueones);
