...
int NEWnOfEffectLits(Seff *se) {
  ...
  case SEoneof:
    fprintf(stderr,"ERROR: oneof not at top level\n");
    exit(1);
...
eff *NEWgroundeff(Seff *se,int *b) {
  int *ptr,cnt;
...
void NEWgothrough(int i,int sc,int **domains,int *staticrestriction) {
  if(i < nOfBindings) { /* base case, instantiate */
    ...
    /* Create name for the ground action. */
    ...
    /* Ground the effect, assuming oneof is the outermost construct. */

    if(Sactions[sc].effect->t == SEoneof) {
      int N = nOfOneofs(Sactions[sc].effect);
      Sefflist *oneofs = getOneofs(Sactions[sc].effect,NULL);
      actions[nOfActions-1].nOfEffects = N;
      actions[nOfActions-1].effects = (eff *)malloc(N * sizeof(eff *));
      int j;
      for(j=0;j < N;j++) {
	actions[nOfActions-1].effects[j] = NEWgroundeff(oneofs->hd,binding);
	oneofs = oneofs->tl;
      }
    } else {
      actions[nOfActions-1].nOfEffects = 1;
      actions[nOfActions-1].effects = (eff *)malloc(sizeof(eff *));
      actions[nOfActions-1].effects[0] = NEWgroundeff(Sactions[sc].effect,binding);
    }
...
void NEWgroundoperators() {
  ...
  /* Go through all schematic actions, and instantiate each. */
  ...
  for(i=0;i<nOfSActions;i++) {
    ...
    if(flagShowInput) {
      ...
      Sactions[i].effect = NFoneof(Sactions[i].effect);
    ...
    /* Go through all parameter assignments and ground the action. */
