...
Seff* Seffconjunction(Sefflist *efs) {
  if(efs && (efs->tl == NULL)) return efs->hd;
...
void checkrequirements(intlist *l) {
  while(l != NULL) {
    ...
    } else if(strcmp(symbol(l->hd),":non-deterministic") == 0) {
    } else {
...
Sefflist *Seffappend(Sefflist *l1,Sefflist *l2) {
  if(l1 == NULL) return l2;
  return Seffcons(l1->hd,Seffappend(l1->tl,l2));
}
