...
class Dag:
    ...
    def write(self):
        ...
        with open(self.problem.tmpDir + "/tmp_dagster_info.json", "w") as outfile:
            ...
            for dagNode in self.graph.nodes():
                ...
                if add:
                    ...
                    '''
                    print("PRINTING FOR SUBPROBLEMs: ", subproblem)
                    print("propositions dealing with", [self.problem.symbols[x] for x in self.subproblemToPropositions[subproblem]])
                    print("actions dealing with", [self.problem.symbols[x] for x in self.subproblemToActions[subproblem]])
                    print("assumptions to add", [self.problem.symbols[x] for x in subproblemToAssumptionsWithoutExtra[subproblem]])
                    '''
        ...
        with open(self.problem.tmpDir + "/tmp_regular.cnf", 'w') as cnfFile2:
            clausesToWrite = []
            clausesToWrite.extend([self.problem.clauses[CR] for CR in self.problem.TCRss[0]])
            clausesToWrite.extend([self.problem.clauses[CR] for CR in self.problem.UCRss[0]])

            numClauses = len(clausesToWrite)
            numVariables = self.problem.totalPerTimestep * 2

            cnfFile2.write("p cnf " + str(numVariables) + " " + str(numClauses) + "\n")
            cnfFile2.write(self.problem.cnfDimacsString(clausesToWrite))
        ...
        with open(self.problem.tmpDir + "/tmp_decomposed.dag", 'w') as dagFile:
            ...
            print("TODO change for debugging")
            satReportingVariables = range(1,self.problem.totalPerTimestep*2+1)
    ...
    @classmethod
    def fromSCCGraph(cls, problem, baseSCCGraph, indexToBaseSCCGraph, SCCGraph, subproblem, layer, extraAssumptions, clauseValidatingLits): # SCCNodeToIndex, indexToSCCNode):
        newDag = Dag(problem, baseSCCGraph, indexToBaseSCCGraph)
        newDag.overwriteWithSCCGraph(SCCGraph, subproblem, layer, extraAssumptions, clauseValidatingLits)
        return newDag
    
    @classmethod
    def soleNode(cls, problem, layers): # SCCNodeToIndex, indexToSCCNode):
        ...
        newDag = Dag(problem, baseSCCGraph, indexToBaseSCCGraph)
        newDag.overwriteWithSoleNode()
        for layer in range(layers):
            baseDag.extend(newDag.getIncrementedLayerCopy(layer))

        return baseDag

    def overwriteWithSoleNode(self):
    ...
    def overwriteWithSCCGraph(self, SCCGraph, subproblem, layer, extraAssumptions, clauseValidatingLits):
        print("MODIFIED")
        ...
        x = time.time()
        # self.SCCNodeToCumulativeActions
        self.computeSCCNodeToCumulativeActions()
        ...
        #rootNode, collatingNode = 
        #self.getRootCollatingNodes()

        # populate dictionaries for format
        self.dagNodeToClauses = {}
        for node in range(len(self.newDag.nodes())):
            self.dagNodeToClauses[0] = RangeSet()
            self.dagNodeToClauses[0].update(self.newTCRss[node])
            self.dagNodeToClauses[0].update(self.newUCRss[node])
        ...
        self.graph = nx.DiGraph()
        self.graph.add_node(0);
        self.subproblemLayerToRootDagNode[(subproblem, layer)] = 0
        self.collatingDagNodeToSubproblem[0] = subproblem
        self.collatingDagNodeToLayer[0] = layer
        self.dagNodeToNogood = {}
        for i in range(len(self.dagCumulativePropositions)):
            self.dagNodeToNogood[0] = [0] #sorted(self.dagCumulativePropositions[i])

        self.dagNodeToBaseSCCNode = {}
        for dagNode in range(len(self.graph.nodes())):
            self.dagNodeToBaseSCCNode[0] = None
        ...
        self.subproblemLayerBaseSCCNodeToDagNode[(subproblem, layer, -1)] = 0
